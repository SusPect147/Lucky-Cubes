...
function enterRainbowMode() {
    ...
    // Меняем базовый кубик на супер-версию
    const defaultGif = isRainbowMode ? 'супер-начальный-кубик.gif' : 'начальный-кубик.gif';
    if (!isCubeAnimating) {
        cubeImg.src = `${basePath}${defaultGif}`;
    }
}
...
function exitRainbowMode() {
    ...
    // Возвращаем обычный кубик
    if (!isCubeAnimating) {
        cubeImg.src = `${basePath}начальный-кубик.gif`;
    }
}
...
const cubeImg = document.getElementById('cube-gif');
const coinCountEl = document.getElementById('coin-count');
const minNumberEl = document.getElementById('min-number');
const basePath = 'UI/кубики/классический%20скин/';
let defaultGif = 'начальный-кубик.gif'; // Будет переопределяться при rainbow
...
cube.addEventListener('click', () => {
    ...
    const rollGif = `${roll}-кубик.gif`;

    cubeImg.src = `${basePath}${rollGif}`;
    ...
    setTimeout(() => {
        // Возвращаем нужный базовый кубик в зависимости от режима
        const currentDefault = isRainbowMode ? 'супер-начальный-кубик.gif' : 'начальный-кубик.gif';
        cubeImg.src = `${basePath}${currentDefault}`;
